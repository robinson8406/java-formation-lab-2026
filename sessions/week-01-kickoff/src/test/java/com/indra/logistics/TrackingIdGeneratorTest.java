...
class TrackingIdGeneratorTest {
    ...
    @Test
    @DisplayName("Debe generar un tracking id con el formato ORIG-DEST-XXXXXXXX")
    ...
    @Test
    @DisplayName("Debe normalizar origin y destination a mayúsculas")
    void shouldNormalizeOriginAndDestinationToUppercase() {
        String trackingId = trackingIdGenerator.generate(" CTG ", "BOG");
        assertTrue(trackingId.matches("^CTG-BOG-[A-Z0-9]{8}$"));
    }

    @ParameterizedTest(name = "Debe lanzar IllegalArgumentException para origin={0} y destination={1}")
