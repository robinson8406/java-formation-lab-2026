...
public class MastercardPayment implements PaymentStrategy {
    ...
    private static final String METHOD_CODE = "MASTERCARD";
    ...
    public BigDecimal calculateFee(BigDecimal amount) {
        if (amount.compareTo(FEE_THRESHOLD) < 0) {
            messageHolder.set("Pago con tarjeta de crédito Mastercard procesado, monto no aplica comisión bancaria.");
            return BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP);
        }
        messageHolder.set("Pago con tarjeta de crédito Mastercard procesado, se aplica comisión bancaria.");
