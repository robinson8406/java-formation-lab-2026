...
public class VisaPayment implements PaymentStrategy {
    ...
    private static final String METHOD_CODE = "VISA";
    private static final BigDecimal FEE_THRESHOLD = BigDecimal.valueOf(100);
    private static final BigDecimal COMMISSION_RATE = BigDecimal.valueOf(0.035);
    ...
    public BigDecimal calculateFee(BigDecimal amount) {
        if (amount.compareTo(FEE_THRESHOLD) < 0) {
            messageHolder.set("Pago con tarjeta de crédito Visa procesado, monto no aplica comisión bancaria.");
            return BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP);
        }
        messageHolder.set("Pago con tarjeta de crédito Visa procesado, se aplica comisión bancaria.");
